...
- Resume reading: reopening a file from Recent returns the reader to the page, layout, and zoom level they left it on. Governed by a Settings preference ("Continue where I left off" — the default — vs. "Always start at the beginning"), so a reader who prefers every document to open at page 1 can have that. Position is stored per document alongside its recent-files entry, so it is pruned automatically when the file falls off the 10-item list.
