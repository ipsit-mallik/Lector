...
- [ ] **8.4 — Contextual voice command router.** Generalizes the idle/active grammar split already in `features/voice/wake.py` into a router that tracks the active screen/dialog (`home`, `reading`, `settings`, `save_dialog`, `open_dialog`, `picker`, `dictation`, extendable) and swaps the active grammar to global commands ∪ current context's scoped commands. Global commands include "undo"/"redo" (wired to the existing `undo_highlight`/`redo_highlight`), "help"/"what can I say", "go home", and (once 8.1 exists) "close app". Migrate the existing reading-view grammar into this router as the `reading` context without changing its behavior.
