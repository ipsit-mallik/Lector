...
- [ ] **8.3 — Confidence-based clarification loop.** Today a phrase that misses `fuzzy.best_match()`'s threshold just does nothing. Add a middle tier: a near-miss score or low Vosk result-confidence triggers a "did you mean '<candidate>'?" clarification instead of silent discard; check n-best alternatives against the active grammar before falling back to clarification.
